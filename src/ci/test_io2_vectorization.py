...
def create_module_pff(path: Path, n_frames: int = 10):
    header_base = (
...
def test_metadata_structuring(complex_run):
...
def test_housekeeping_parsing(complex_run):
    run = PanosetiRun(complex_run)
    hk = run.get_hk()
    assert hk["QUABO_0"]["DET_TEMP"] == [45.5]

def process_run_func(r):
    p_name = r.list_products()[0]
    s = r.get_product(p_name)
    return len(s)

def test_multiprocessing_run(complex_run):
    run = PanosetiRun(complex_run)
